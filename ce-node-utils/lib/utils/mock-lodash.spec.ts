...
describe('OmitTest', () => {
  it('native', () => {
    ...
    // `rank` and `age`.
    const newObj: { [key: string]: any } = (({ rank, age, ...rest }) => rest)(
      obj,
    )
  ...
  it('omit', () => {
    ...
    // function that uses the spread operator to get all properties _other than_
    // `rank` and `age`.
    const newObj = omit(obj, ['rank', 'age']) as any
...
describe('DifferenceTest', () => {
  ...
  it('DifferenceWith', () => {
    ...
    const values = [obj2]
    expect(isEqual(differenceWith(array, values, isEqual), [obj1])).toBeTruthy()
  })
})
...
describe('PickTest', () => {
  it('pick', () => {
    ...
    const picked: Record<string, any> = pick(obj, ['name', 'rank'])
  ...
  it('pick by path', () => {
    ...
    const picked = pick(obj, ['name.last', 'rank', 'this.is.not.in.the.object'])
